  ...
  return `
        <div class="garden-card ${urgencyClass}" style="display: flex; align-items: center;">
            ...
            <div class="garden-info" style="flex: 1;">
                <h3 style="margin: 0 0 4px 0; font-size: 1rem;">${
                  plant.nickname
                }</h3>
                <p style="margin: 0; font-size: 0.8rem; opacity: 0.8;">${
                  plant.originalName
                }</p>
                <div style="margin-top:5px; font-weight:bold; font-size: 0.85rem; color:${
            ...
            <div class="garden-actions" style="margin-right: 5px;">
                <button class="btn-water" style="background:var(--secondary-green); padding: 6px 10px;" onclick="app.openDiary(${
                  plant.id
        ...
        <div style="text-align: left; margin-top: -10px; margin-bottom: 10px; padding-left: 10px;">
             <small onclick="app.deletePlant(${
               plant.id
             })" style="color: var(--warning-red); cursor: pointer; opacity: 0.7;">حذف گیاه <i class="fas fa-trash"></i></small>
        </div>
    `;
}
